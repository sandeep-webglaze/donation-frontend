    ...
    <section className="container py-20 md:py-28">
      <div className="mx-auto max-w-2xl text-center">
        ...
        <p className="mt-4 text-muted-foreground">
          From rural health camps to heritage preservation — a look at the work your support makes possible.
        </p>
      </div>
        ...
        {items.length > 0 ? (
          ...
          <div className="grid grid-cols-2 gap-4 md:grid-cols-3">
            {PLACEHOLDERS.map((t) => {
              const Icon = t.icon;
              return (
                <div key={t.label} className={`group relative flex aspect-[4/3] items-center justify-center overflow-hidden rounded-2xl border bg-gradient-to-br ${t.grad}`}>
                  <Icon className="h-10 w-10 text-foreground/30 transition-transform group-hover:scale-110" />
                  <span className="absolute bottom-3 left-4 text-sm font-medium text-foreground/70">{t.label}</span>
                </div>
              );
            })}
          </div>
